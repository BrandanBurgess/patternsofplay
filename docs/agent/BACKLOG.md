...
|---|---|---|---|---|---|---|---|
| T-001 | Scaffold: FastAPI+SQLAlchemy+Alembic+SQLite(WAL), React+Vite, Makefile (bootstrap/dev/lint/typecheck/test/e2e/verify/seed), CI workflow, e2e harness from e2e/fixtures.ts | 1 | platform | sonnet | none | none (solo, first) | todo |
| T-002 | Design tokens: 3 themes as CSS vars on html[data-theme], Oswald+Inter, switcher | 2 | screens | sonnet | T-001 | T-003, T-004 | todo |
| T-003 | Auth + teams: register, roles, team create, join code, join flow, minimal token-styled screens (Brief §8: invent nothing) | 3 | platform | sonnet | T-001 | T-002 | todo |
...
| T-020 | Board core: pitch canvas, landscape model coords, token drag 60fps @23 tokens, portrait mapping (left=y, top=100-x) with lossless round-trip unit test FIRST | 9, 10 | board-engineer | opus | T-001 | T-010 | todo |
